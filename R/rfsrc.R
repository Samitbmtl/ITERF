iterf <- function(formula, data, ntree = 1000,
                  mtry = NULL, ytry = NULL,
				  ...
				  #Sami
				  iterf.tailType = 1,
				  iterf.minObsControl = 10,
				  iterf.minObsTreatment = 10,
				  iterf.TreatmentLowBound = 0.3,
				  iterf.TreatmentUpperBound = 0.7,
                   ...
                   ...)
{
    ...
    if (impute.only) {
        ...
        ## impute.iterf( generic.impute.iterf( iterf() ) ).  Now, the user specified option
        ## is respected.  Note that calls from impute.iterf()
    ...
    ##Sami
	  contol.iterf(family,nodesize,iterf.tailType,iterf.minObsControl,iterf.minObsTreatment,iterf.TreatmentLowBound,iterf.TreatmentUpperBound)
	  if (iterf.nbCore <= 0) 
	    iterf.nbCore <- -1
    ...
    nativeOutput <- tryCatch({.Call("rfsrcGrow",
                                    ...
                                    #as.double(prob.epsilon), ##replace with upper line. cant send null. otherwise will create a r shutdown
                                    as.double(NULL),
                                    #as.integer(get.rf.cores()),
                                    as.integer(iterf.nbCore),
                  									as.integer(iterf.tailType),
                  									as.integer(iterf.minObsControl),
                  									as.integer(iterf.minObsTreatment),
                  									as.double(iterf.TreatmentLowBound),
                  									as.double(iterf.TreatmentUpperBound)
                  									)}, error = function(e) {
                                        print(e)
                                        NULL})
    ...
    if (is.null(nativeOutput)) {
        if (impute.only) {
            ...
            ## currently all other modes will error out with the iterf error message
    ...
    if (forest) {
      ...
      class(forest.out) <- c("iterf", "forest", family)
      if (big.data) {
  ...
  if (proximity != FALSE) {
	...
	iterf <- list(tailType = iterf.tailType,
						minObsControl = iterf.minObsControl
						,minObsTreatment = iterf.minObsTreatment
						,TreatmentLowBound = iterf.TreatmentLowBound
						,TreatmentUpperBound = iterf.TreatmentUpperBound)
    ...
    if (!impute.only) {
        ...
        if (grepl("surv", family) || grepl("IndSurvB", family) || grepl("HET", family) || grepl("indQuadC", family) || grepl("indQuad2C", family) || grepl("indLSC", family) || grepl("indTSTC", family) || grepl("indTST2C", family) || grepl("indNSRC", family) || grepl("indMOB0C", family) || grepl("CMB", family) || grepl("indMOBQC", family) || grepl("indMaxLC", family) || grepl("indMaxQC", family) || grepl("indMaxMaxQC", family) || grepl("indCat", family) || grepl("IndSurvCat", family)) {
            ...
            if (!is.null(nativeOutput$holdoutSurv)) {
                holdout.vimp <- vector("list", length(iterfOutput$holdout.blk))
                names(holdout.vimp) <- xvar.names
                ## These are lengths of of each x-var dimension.    
                holdout.offset <- iterfOutput$holdout.blk * length(event.info$event.type) 
                holdout.offset.sum <- c(0, cumsum(holdout.offset))
                for (i in 1:length(holdout.vimp)) {
                    if (iterfOutput$holdout.blk[i] > 0) {
                        if (length(event.info$event.type) > 1) {
                            holdout.vimp[[i]] <- array(nativeOutput$holdoutSurv[(holdout.offset.sum[i] + 1) : holdout.offset.sum[i+1]], c(length(event.info$event.type), iterfOutput$holdout.blk[i]))
                        }
                        else {
            ...
            if (resp.clas.count > 0) {
                ...
                if (!is.null(nativeOutput$holdoutClas)) {
                    ## These are lengths of of each x-var dimension.    
                    holdout.offset.x <- iterfOutput$holdout.blk * (sum(1 + levels.count))
                ...
                for (i in 1:resp.clas.count) {
                    ...
                    if (!is.null(nativeOutput$holdoutClas)) {    
                        ...
                        for (k in 1:length(holdout.vimp)) {
                            ...
                            if (iterfOutput$holdout.blk[k] > 0) {
                                ...
                                for (m in 1:iterfOutput$holdout.blk[k]) {
                                    if (m > 1) {
                                ...
                                holdout.vimp[[k]] = array(nativeOutput$holdoutClas[index.m], c(levels.count[i] + 1, iterfOutput$holdout.blk[k]))
                            }
                            else {
                                holdout.vimp[[k]] = NA
                            }
            ...
            if (resp.regr.count > 0) {
                ...
                if (!is.null(nativeOutput$holdoutRegr)) {
                    ## These are lengths of of each x-var dimension.    
                    holdout.offset.x <- iterfOutput$holdout.blk * resp.regr.count
                    ## This is the cumulative offset for each x-var dimension.
                    holdout.offset.sum.x <- c(0, cumsum(holdout.offset.x))
                    ## Relative response offset.
                    holdout.offset.r <- 0
    ...
    class(iterfOutput) <- c("iterf", "grow", family)
    if (big.data) {
        class(iterfOutput) <- c(class(iterfOutput), "bigdata")
    }
    return(iterfOutput)
}
