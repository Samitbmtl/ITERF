           ...
           ...)
{
  ...
  if (!missing(object)) {
    ...
    if (sum(inherits(object, c("iterf", "synthetic"), TRUE) == c(1, 2)) != 2) {
      stop("this function only works for objects of class `(iterf, synthetic)'")
    }
    ...
    preObj <- iterf(f.org, data, ntree = 1, nodesize = nrow(data), splitrule = "random")
    fmly <- preObj$family
    ...
    ## determine the grow interface - iterf or iterf.fast?
    if (!fast) {
      iterf.grow <- "iterf"
    }
    else {
      iterf.grow <- "iterf.fast"
    }
  }
  ...
  if (missing(object)) {
    ...
    if (oob) {
      ...
      ## inbag sample depends on whether iterf.fast is requested or default sampling used by iterf
      ## note that this is now "swor" in both cases
      roughO <- do.call("iterf",
  ...
  if (oob) {
    class(retObj) <- c("iterf", "synthetic", "oob")
  }
  else {
    class(retObj) <- c("iterf", "synthetic", "inb")
  }
  retObj
}
## --------------------------------------------------------------
