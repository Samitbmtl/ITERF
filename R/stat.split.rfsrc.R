stat.split.iterf <- function(object, ...)
{
  ...
  if ((sum(inherits(object, c("iterf", "grow"), TRUE) == c(1, 2)) != 2) &&
      (sum(inherits(object, c("iterf", "predict"), TRUE) == c(1, 2)) != 2))
    stop("This function only works for objects of class `(iterf, grow)' or `(iterf, pred)',")
