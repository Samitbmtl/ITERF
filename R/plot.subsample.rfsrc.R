plot.subsample.iterf <- function(x, alpha = .01,
                          ...
                          ...)
{
  ...
  ##
  ##--------------------------------------------------------------
  if (sum(c(grepl("iterf", class(x))), grepl("subsample", class(x))) == 2) {
    subsample <- TRUE
  }
  else if (sum(c(grepl("iterf", class(x))), grepl("bootsample", class(x))) == 2) {
    subsample <- FALSE
  }
  else {
...
plot.subsample <- plot.subsample.iterf
