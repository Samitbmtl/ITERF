                            ...
                            verbose = TRUE) 
{
  ...
  ##
  ##--------------------------------------------------------------
  if (sum(inherits(obj, c("iterf", "grow"), TRUE) == c(1, 2)) != 2) {
...
extract.subsample <- function(obj, alpha = .05, target = 0, m.target = NULL, standardize = TRUE)
{
  ...
  if (sum(c(grepl("iterf", class(obj))), grepl("subsample", class(obj))) == 2) {
    subsample <- TRUE
  }
  else if (sum(c(grepl("iterf", class(obj))), grepl("bootsample", class(obj))) == 2) {
    subsample <- FALSE
  }
  else {
...
##
###################################################################
print.subsample.iterf <- function(x, alpha = .05, standardize = TRUE) {
  vmp <- x$vmp
...
print.subsample <- print.subsample.iterf
###################################################################
##
...
extract.bootsample <- function(obj, alpha = .05, target = 0, m.target = NULL, standardize = TRUE)
{
  ...
  if (!sum(c(grepl("iterf", class(obj))), grepl("bootsample", class(obj))) == 2) {
...
##
###################################################################
print.bootsample.iterf <- function(x, alpha = .05, standardize = TRUE) {
  vmp <- x$vmp
  nullO <- lapply(1:length(vmp), function(j) {
    m.target <- names(vmp)[j]
    p.vmp <- ncol(vmp[[j]])
...
print.bootsample <- print.bootsample.iterf
