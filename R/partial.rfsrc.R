  ...
  ...)
{
  ...
  if (sum(inherits(object, c("iterf", "grow"), TRUE) == c(1, 2)) == 2) {
    if (is.forest.missing(object)) {
      stop("The forest is empty.  Re-run iterf (grow call) with forest=TRUE")
    }
  ...
  ## added from generic.predict.iterf.R per UBK instruction 11/04/2019
  if (hdim == 0) {
    offset = 0
    chunk = 0
  } else {
  ...
  iterfOutput <- list(call = match.call(),
                      family = family,
  ...
  class(iterfOutput) <- c("iterf", "partial",   family)
  return (iterfOutput)
}
partial <- partial.iterf
