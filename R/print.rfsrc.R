print.iterf <- function(x, outcome.target = NULL, ...) {
  ## default printing
  if (sum(inherits(x, c("iterf", "forest"), TRUE) == c(1, 2)) == 2) {
    print.default(x)
    return()
  }
  if(sum(inherits(x, c("iterf", "plot.variable"), TRUE) == c(1, 2)) == 2) {
    print.default(x)
    return()
  }
  if (sum(inherits(x, c("iterf", "partial"), TRUE) == c(1, 2)) == 2) {
    print.default(x)
    return()
  }
  if (sum(inherits(x, c("iterf", "sidClustering"), TRUE) == c(1, 2)) == 2) {
    print.default(x)
    return()
  }
  ...
  if (sum(inherits(x, c("iterf", "synthetic"), TRUE) == c(1, 2)) == 2) {
    if (sum(inherits(x, c("iterf", "synthetic", "oob"), TRUE) == c(1, 2, 3)) != 3) {
      sf.flag <- TRUE
  ...
  if (sum(inherits(x, c("iterf", "subsample"), TRUE) == c(1, 4)) == 2) {
    print.subsample(x, ...)
    return()
  }
  ## is this a subsampled-bootstrap object?
  if (sum(inherits(x, c("iterf", "bootsample"), TRUE) == c(1, 4)) == 2) {
    print.bootsample(x, ...)
    return()
  }
  ## check that the object is interpretable
  if (sum(inherits(x, c("iterf", "grow"), TRUE) == c(1, 2)) != 2 &
      sum(inherits(x, c("iterf", "predict"), TRUE) == c(1, 2)) != 2) {
    stop("This function only works for objects of class `(iterf, grow)' or '(iterf, predict)'.")
  }
  ## are we in grow mode?
  if (sum(inherits(x, c("iterf", "grow"), TRUE) == c(1, 2)) == 2) {
    grow.mode <- TRUE
