           ...
           )
{
  ...
  ##
  ## --------------------------------------------------------------
  iterf.var.hunting <- function(train.id, var.pt, nstep) {
    ## ------------------filtering step-----------------------
    ...
    iterf.filter.obj  <- iterf(iterf.all.f,
    ...
    if (iterf.filter.obj$family == "surv-CR") {
      target.dim <- max(1, min(cause, max(get.event.info(iterf.filter.obj)$event.type)), na.rm = TRUE)
    }
    ## extract vimp
    ## for multivariate families we must manually extract the importance and error rate
    imp <- get.varselect.imp(coerce.multivariate(iterf.filter.obj, m.target), target.dim)
    names(imp) <- iterf.filter.obj$xvar.names
    ...
    ## selection using minimal depth
        ...
        max.obj <- max.subtree(iterf.filter.obj, conservative = (conservative == "high"))
        if (is.null(max.obj$order)) {
          ...
          sig.vars.old <- names(sort(imp, decreasing = TRUE))[1]
            ...
            names(m.depth) <- names(VarStrength) <- iterf.filter.obj$xvar.names
    ...
    ## set nstep
    nstep <- max(round(length(iterf.filter.obj$xvar.names)/nstep), 1)
    imp.old <- 0
    ...
    for (b in 1:nstep) {
      ...
      imp <- coerce.multivariate(vimp(iterf.filter.obj, sig.vars, m.target = m.target,
    ...
    iterf.obj  <- iterf(iterf.all.f,
                        ...
                        na.action = na.action,
                        perf.type = perf.type)
    return(list(iterf.obj=iterf.obj, sig.vars=iterf.obj$xvar.names, forest.depth=forest.depth, m.depth=m.depth))
  }
  ## --------------------------------------------------------------
  ##   
  ...
  if (missing(object)) {
    ## parse the formula
    formulaDetail <- finalizeFormula(parseFormula(iterf.all.f, data), data)
  ...
  if (method == "md") {
    ...
    if (prefit.flag && is.null(xvar.wt) && missing(object)) {
      ...
      iterf.prefit.obj  <- iterf(iterf.all.f,
                                 data = data,
                                 ntree = prefit$ntree,
                                 ...
                                 block.size = block.size, perf.type = perf.type)
      ## set the target dimension for CR families
      if (iterf.prefit.obj$family == "surv-CR") {
        target.dim <- max(1, min(cause, max(get.event.info(iterf.prefit.obj)$event.type)), na.rm = TRUE)
      }
      ## for multivariate families we must manually extract the importance and error rate
      wts <- pmax(get.varselect.imp(coerce.multivariate(iterf.prefit.obj, m.target), target.dim), 0)
      if (any(wts > 0)) {
    ...
    ## otherwise run forests...then extract minimal depth    
        ...
        iterf.obj <- iterf(iterf.all.f,
                           ...
                           block.size = block.size, perf.type = perf.type)
        ## set the target dimension for CR families
        if (iterf.obj$family == "surv-CR") {
          target.dim <- max(1, min(cause, max(get.event.info(iterf.obj)$event.type)), na.rm = TRUE)
        }
        if (verbose) cat("minimal depth variable selection ...\n")
        md.obj <- max.subtree(iterf.obj, conservative = (conservative == "high"))
        ## for multivariate families we must manually extract the importance and error rate
        iterf.obj <- coerce.multivariate(iterf.obj, m.target)
        m.target <- iterf.obj$outcome.target
        pe <- get.varselect.err(iterf.obj)
        imp <- get.varselect.imp(iterf.obj, target.dim)
        imp.all <- get.varselect.imp.all(iterf.obj)
        mtry <- iterf.obj$mtry
        nodesize <- iterf.obj$nodesize
        n <- nrow(iterf.obj$xvar)
        family <- iterf.obj$family
        rm(iterf.obj)#don't need the grow object
      }
    ...
    if (refit == TRUE) {
      ...
      drop.var.pt <- setdiff(var.columns, var.pt)
      iterf.refit.obj  <- iterf(iterf.all.f,
                                ...
                                perf.type = perf.type)
      ## for multivariate families we must manually extract the importance and error rate
      iterf.refit.obj <- coerce.multivariate(iterf.refit.obj, m.target)
    }
      else {
        iterf.refit.obj <- NULL
      }
    ...
    return(invisible((list(err.rate=pe,
                           ...
                           topvars=topvars,
                           varselect=varselect,
                           iterf.refit.obj=iterf.refit.obj,
                           md.obj=md.obj
                           ))))
  }  
  ## --------------------------------------------------------------
  ...
  for (m in 1:nrep) {
    ...
    if (is.null(xvar.wt)) {
      ...
      ## for multivariate families we must manually extract the importance and error rate
      iterf.prefit.obj <- coerce.multivariate(iterf.prefit.obj, m.target)
      wts <- pmax(get.varselect.imp(iterf.prefit.obj, target.dim), 0)
      if (any(wts > 0)) {
    ...
    ## iterf gene hunting call
    object <- iterf.var.hunting(train.id, var.pt, nstep)
    iterf.obj <- object$iterf.obj
    m.target <- get.univariate.target(iterf.obj, m.target)
    ...
    ## iterf prediction
    ## for multivariate families we must manually extract the importance and error rate
    pred.out <- coerce.multivariate(predict(iterf.obj, data[test.id, ]), m.target)
