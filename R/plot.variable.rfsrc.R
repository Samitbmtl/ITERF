  ...
  ...)
{
  ...
  if (sum(inherits(x, c("iterf", "synthetic"), TRUE) == c(1, 2)) == 2) {
    x <- x$rfSyn
  }
  ...
  if (sum(inherits(object, c("iterf", "grow"), TRUE) == c(1, 2)) != 2 &
      sum(inherits(object, c("iterf", "plot.variable"), TRUE) == c(1,2)) != 2) {
    stop("this function only works for objects of class `(iterf, grow)', '(iterf, synthetic)' or '(iterf, plot.variable)'")
  }
  ...
  if (partial && (is.null(object$forest) || !object$forest$forest)) {
    stop("forest is empty:  re-run iterf (grow call) with forest=TRUE")
  }
  ...
  if (!inherits(object, "plot.variable")) {
      ...
      yhat <- extract.pred(predict(object, m.target = m.target),
        ...
        prtl <- lapply(1:nvar, function(k) {
          ...
          if (!is.factor(x) & n.x > npts) {
            ...
            if (is.factor(x)) {
              x.uniq <- 1:length(unique(x))##partial.iterf requires factors as integers
            }
            else {
          ...
          pred.temp <- extract.partial.pred(partial.iterf(object$forest,
                                                          m.target = m.target,
    ...
    ## assign the class
    class(plot.variable.obj) <- c("iterf", "plot.variable", family)
  }
  ## ---------------------------------------------------------------------------------
...
## using the partial.iterf() wrapper, and then extracted without any
...
plot.variable <- plot.variable.iterf
