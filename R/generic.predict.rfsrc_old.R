           ...
           ...)
{
  ...
  if (sum(inherits(object, c("iterf", "grow"), TRUE) == c(1, 2)) != 2    &
      sum(inherits(object, c("iterf", "forest"), TRUE) == c(1, 2)) != 2)
    stop("this function only works for objects of class `(iterf, grow)' or '(iterf, forest)'")
  ## grow forests must have true forest information
  if (sum(inherits(object, c("iterf", "grow"), TRUE) == c(1, 2)) == 2) {
    if (is.forest.missing(object)) {
      stop("Forest information for prediction is missing.  Re-run iterf (grow call) with forest=TRUE")
    }
  }
  ...
  ## pivot point, refer to iterf.R, in the POST PROCESSING section,
  ## after $nativeArray has been defined.  This is the structure
  ## that is parsed below.  Prior code in iterf.R is parsing the
  ...
  if (grepl("surv", family) || grepl("IndSurvB", family) || grepl("HET", family) || grepl("indQuadC", family) || grepl("indQuad2C", family) || grepl("indLSC", family) || grepl("indTSTC", family) || grepl("indTST2C", family) || grepl("indNSRC", family)  || grepl("indMOB0C", family) || grepl("CMB", family) || grepl("indMOBQC", family) || grepl("indMaxLC", family) || grepl("indMaxQC", family) || grepl("indMaxMaxQC", family)) {
	...
	##Sami
	if (grepl("IndSurvB", iterfOutput$family))
	{
		#predicted <- Reduce(c,parallel::mclapply(1:nrow(iterfOutput$membership), fun_pred,Objyvar,ObjMembership,iterfOutput$membership,Objinbag))

# 	  no_cores <-parallel::detectCores()
# 	  cl_compile<- parallel::makeCluster(no_cores)
# 	  doParallel::registerDoParallel(cl_compile)
# 	  predicted <- foreach::foreach(i=1:nrow(iterfOutput$membership), .combine = rbind) %dopar% 
#     {
#       fun_pred(i,Objyvar,ObjMembership,iterfOutput$membership,Objinbag)
#     }
# 	  parallel::stopCluster(cl_compile)
	  
	  	  no_cores <-parallel::detectCores()
	  	  cl_compile<- parallel::makeCluster(no_cores)
	  	  predicted <- parallel::parSapply(cl_compile,1:nrow(iterfOutput$membership), fun_pred,Objyvar,ObjMembership,iterfOutput$membership,Objinbag)
	  	  parallel::stopCluster(cl_compile)
	}
	if (grepl("HET", iterfOutput$family))
	{
	  #print("C Ok")
	  	  no_cores <-parallel::detectCores()
	  	  cl_compile<- parallel::makeCluster(no_cores)
	  	  #predicted <- parallel::parSapply(cl_compile,1:1, fun_pred_5beta,Objyvar,ObjMembership,iterfOutput$membership,Objinbag)
	  	  #predicted <- parallel::parSapply(cl_compile,1:nrow(iterfOutput$membership), fun_pred_indLinearC,Objyvar,ObjMembership,iterfOutput$membership,Objinbag)
	  	  predicted <- data.frame(t(parallel::parSapply(cl_compile,1:nrow(iterfOutput$membership), fun_pred_5beta,Objyvar,ObjMembership,iterfOutput$membership,Objinbag)))
	...
	if (grepl("indQuadC", iterfOutput$family))
	{
	  #print("C Ok")
	  	  no_cores <-parallel::detectCores()
	  	  cl_compile<- parallel::makeCluster(no_cores)
	  	  #predicted <- parallel::parSapply(cl_compile,1:nrow(iterfOutput$membership), fun_pred_indQuadC,Objyvar,ObjMembership,iterfOutput$membership,Objinbag)
	  	  predicted <- data.frame(t(parallel::parSapply(cl_compile,1:nrow(iterfOutput$membership), fun_pred_5beta,Objyvar,ObjMembership,iterfOutput$membership,Objinbag)))
	  	  #names(predicted) <- c("b0int","b0lin","b1lin","b0quad","b1quad","b2quad","N_b0int","N_b0lin","N_b1lin","N_b0quad","N_b1quad","N_b2quad")
	  	  names(predicted) <- c("b0int","b0lin","b1lin","b0quad","b1quad","b2quad")
	  	  parallel::stopCluster(cl_compile)
	...
	if (grepl("indQuad2C", iterfOutput$family))
	{
	  #print("C Ok")
	  	  no_cores <-parallel::detectCores()
	  	  cl_compile<- parallel::makeCluster(no_cores)
	  	  #predicted <- parallel::parSapply(cl_compile,1:nrow(iterfOutput$membership), fun_pred_indQuad2C,Objyvar,ObjMembership,iterfOutput$membership,Objinbag)
	  	  predicted <- data.frame(t(parallel::parSapply(cl_compile,1:nrow(iterfOutput$membership), fun_pred_5beta,Objyvar,ObjMembership,iterfOutput$membership,Objinbag)))
	  	  #names(predicted) <- c("b0int","b0lin","b1lin","b0quad","b1quad","b2quad","N_b0int","N_b0lin","N_b1lin","N_b0quad","N_b1quad","N_b2quad")
	  	  names(predicted) <- c("b0int","b0lin","b1lin","b0quad","b1quad","b2quad")
	  	  parallel::stopCluster(cl_compile)
	...
	if (grepl("indLSC", iterfOutput$family))
	{
	  #print("C Ok")
		no_cores <-parallel::detectCores()
		cl_compile<- parallel::makeCluster(no_cores)
	...
	if (grepl("indTSTC", iterfOutput$family) || grepl("indTST2C", iterfOutput$family))
	{
	  #print("C Ok")
		no_cores <-parallel::detectCores()
		cl_compile<- parallel::makeCluster(no_cores)
	...
	if (grepl("indNSRC", iterfOutput$family))
	{
	  #print("C Ok")
		no_cores <-parallel::detectCores()
		cl_compile<- parallel::makeCluster(no_cores)
	...
	if (grepl("indMOB0C", iterfOutput$family))
	{
	  #print("C Ok")
		no_cores <-parallel::detectCores()
		cl_compile<- parallel::makeCluster(no_cores)
	...
	if (grepl("CMB", iterfOutput$family))
	{
	  #print("C Ok")
		no_cores <-parallel::detectCores()
		cl_compile<- parallel::makeCluster(no_cores)
	...
	if (grepl("indMOBQC", iterfOutput$family))
	{
	  #print("C Ok")
		no_cores <-parallel::detectCores()
		cl_compile<- parallel::makeCluster(no_cores)
	...
	if (grepl("indMaxLC", iterfOutput$family))
	{
	  #print("C Ok")
		no_cores <-parallel::detectCores()
		cl_compile<- parallel::makeCluster(no_cores)
	...
	if (grepl("indMaxQC", iterfOutput$family))
	{
	  #print("C Ok")
		no_cores <-parallel::detectCores()
		cl_compile<- parallel::makeCluster(no_cores)
	...
	if (grepl("indMaxMaxQC", iterfOutput$family))
	{
	  #print("C Ok")
		no_cores <-parallel::detectCores()
		cl_compile<- parallel::makeCluster(no_cores)
	...
	##Sami
	if (grepl("IndSurvB", iterfOutput$family) || grepl("HET", iterfOutput$family) || grepl("indQuadC", iterfOutput$family) || grepl("indQuad2C", iterfOutput$family) || grepl("indLSC", iterfOutput$family) || grepl("indTSTC", iterfOutput$family) || grepl("indTST2C", iterfOutput$family) || grepl("indNSRC", iterfOutput$family)  || grepl("indMOB0C", iterfOutput$family) || grepl("CMB", iterfOutput$family) || grepl("indMOBQC", iterfOutput$family) || grepl("indMaxLC", iterfOutput$family) || grepl("indMaxQC", iterfOutput$family) || grepl("indMaxMaxQC", iterfOutput$family))
	{
		predicted.oob <- NULL
	}
    ...
    if(univariate.nomenclature) {
      iterfOutput <- c(iterfOutput, survOutput)
    }
      else {
        iterfOutput <- c(iterfOutput, survOutput = list(survOutput))
      }
  }
    else {
      ...
      if (class.count > 0) {
        ...
        if(univariate.nomenclature) {
          if ((class.count == 1) & (regr.count == 0)) {
            ...
            iterfOutput <- c(iterfOutput, unlist(classOutput, recursive=FALSE))
          }
            else {
              iterfOutput <- c(iterfOutput, classOutput = list(classOutput))
            }
        }
          else {
            iterfOutput <- c(iterfOutput, classOutput = list(classOutput))
      ...
      if (regr.count > 0) {
        ...
        if(univariate.nomenclature) {
          if ((class.count == 0) & (regr.count == 1)) {
            ...
            iterfOutput <- c(iterfOutput, unlist(regrOutput, recursive=FALSE))
          }
            else {
              iterfOutput <- c(iterfOutput, regrOutput = list(regrOutput))
            }
        }
          else {
            iterfOutput <- c(iterfOutput, regrOutput = list(regrOutput))
  ...
  class(iterfOutput) <- c("iterf", "predict",   family)
  return(iterfOutput)
}
