...
get.cindex <- function (time, censoring, predicted, do.trace = FALSE) {
  ...
  if (is.null(nativeOutput)) {
    stop("An error has occurred in iterfCIndex.  Please turn trace on for further analysis.")
  }
...
##Sami
contol.iterf <- function (family,nodeMinSize,iterf.tailType,iterf.minObsControl,iterf.minObsTreatment,iterf.TreatmentLowBound,iterf.TreatmentUpperBound) {
	if (grepl("IndSurvB", family)) {
		if (iterf.tailType != 0 && iterf.tailType != 1)
		{
			stop("tail type should be 0 (none) or 1 (Exponential)")
		}
		if (iterf.minObsControl < 1 || iterf.minObsControl > nodeMinSize)
		{
			stop("iterf.minObsControl should be between 1 and nodesize")
		}
		if (iterf.minObsTreatment < 1 || iterf.minObsTreatment > nodeMinSize)
		{
			stop("iterf.minObsTreatment should be between 1 and nodesize")
		}
		if (iterf.TreatmentLowBound < 0 || iterf.TreatmentLowBound > iterf.TreatmentUpperBound)
		{
			stop("iterf.TreatmentLowBound should be between 0 and iterf.TreatmentUpperBound")
		}
		if (iterf.TreatmentUpperBound > 1 || iterf.TreatmentUpperBound < iterf.TreatmentLowBound)
		{
			stop("iterf.TreatmentUpperBound should be between TreatmentLowBound and 1")
		}
	}
...
get.grow.event.info <- function(yvar, fmly, need.deaths = TRUE, ntime) {
  ...
  ## handle new Individualized
	...
	if (max(cens) != 1 && min(cens) != 0) {
		stop("for iterf families censoring should be coded 0/1")
	}
	if (max(group) != 1 && min(group) != 0) {
		stop("for iterf families group should be coded 0/1")
	}
	if (length(sort(unique(cens))) == 2 && (sort(unique(cens))[1] != 0 || sort(unique(cens))[2] != 1))
	{
		stop("for iterf families censoring should be coded 0 or 1")
	}
	if (length(sort(unique(group))) == 2 && (sort(unique(group))[1] != 0 || sort(unique(group))[2] != 1))
	{
		stop("for iterf families group should be coded 0 or 1")
	}
