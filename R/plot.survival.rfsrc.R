plot.survival.iterf <- function (x,
                                 plots.one.page = TRUE,
                                 show.plots = TRUE,
                                 subset, collapse = FALSE,
                                 cens.model = c("km", "iterf"),
                                 ...)
{
  ...
  if (sum(inherits(x, c("iterf", "predict"), TRUE) == c(1, 2)) == 2) {
    pred.flag <- TRUE
  }
  else {
    pred.flag <- FALSE
  ...
  cens.model <- match.arg(cens.model, c("km", "iterf"))
  ...
  if (!pred.flag && !subset.provided) {
    ...
    if (length(censTime) > 0) {
      ...
      ## iterf estimator for the censoring distribution
      else {
        cens.dist <- iterf(Surv(tm, cens) ~ .,
...
plot.survival <- plot.survival.iterf
## trapezoidal rule
trapz <- function (x, y) {
  idx = 2:length(x)
