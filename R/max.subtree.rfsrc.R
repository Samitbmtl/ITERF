max.subtree.iterf <- function(object,
                              max.order = 2,
                              sub.order = FALSE,
                              conservative = FALSE,
                              ...)
{
  ...
  if (is.null(nativeArray)) {
    stop("iterf nativeArray content is NULL.  Please ensure the object is valid.")
  }
  xvar.names <- object$xvar.names
  if (is.null(xvar.names)) {
    stop("iterf xvar.names content is NULL.  Please ensure the object is valid.")
  }
  if (is.null(object$xvar)) {
    stop("iterf xvar content is NULL.  Please ensure the object is valid.")
  }
  ## verify the max.order option
  max.order <- floor(max.order)
  if (max.order < 0) {
    stop("iterf 'max.order' requested for distance order statistic must be an integer greater than zero (0).")
  }
  ...
  subtree.obj <- mclapply(1:numTree, function(b) {
    ...
    recursiveObject <- iterfParseTree(
      recursiveObject,
      max(max.order, 1),
      sub.order,
      nativeArray,
...
iterfParseTree <- function(recursiveObject,
                           max.order,
                           sub.order,
                           nativeArray,
                           b,
  ...
  if (!terminalFlag) {
    ...
    if (subtreeFlag[splitParameter] == FALSE) {
      ...
      if (sub.order == TRUE) {
        ...
        recursive2Object <- iterfParse2Tree(recursive2Object,
                                            nativeArray,
                                            b,
                                            distance=0,
  ...
  if (terminalFlag == FALSE) {
    ...
    distance <- distance + 1
    ## Parse left:
    recursiveObject <- iterfParseTree(recursiveObject, max.order, sub.order, nativeArray, b, distance, subtreeFlag)
    ## Parse right:
    recursiveObject <- iterfParseTree(recursiveObject, max.order, sub.order, nativeArray, b, distance, subtreeFlag)
  }
    else {
...
iterfParse2Tree <- function(recursiveObject,
                            nativeArray,
                            b,
                            distance,
                            subtreeFlag) {
  ...
  if (terminalFlag == FALSE) {
    ## Parse left:
    recursiveObject = iterfParse2Tree(recursiveObject, nativeArray, b, distance, subtreeFlag)
    ## Parse right:
    recursiveObject = iterfParse2Tree(recursiveObject, nativeArray, b, distance, subtreeFlag)
  }
  return(recursiveObject)
