quantileReg.iterf <- function(formula, data, object, newdata, method = "forest",
                              ...
                              oob = TRUE, fast = FALSE, maxn = 1e3, ...)
{
  ...
  if (missing(object)) {
    ...
    object <- do.call(iterf.grow, c(list(formula = formula, data = data, splitrule = splitrule), dots))
  }
