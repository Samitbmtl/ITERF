...
# 	  parallel::stopCluster(cl_compile)
	  ...
	  # for(i in 1:nrow(iterfOutput$membership))
	  # {
	  #   predicted <- fun_pred(i,Objyvar,ObjMembership,iterfOutput$membership,Objinbag, !(OOB))
	  # }
	          
	  	  no_cores <-parallel::detectCores()
	  	  cl_compile<- parallel::makeCluster(no_cores)
	  	  predicted <- parallel::parSapply(cl_compile,1:nrow(iterfOutput$membership), fun_pred,Objyvar,ObjMembership,iterfOutput$membership,Objinbag, !(OOB))
	  	  parallel::stopCluster(cl_compile)
	}
	if (grepl("IndSurvCat", iterfOutput$family))
	{
	  	  # no_cores <-parallel::detectCores()
	  	  # cl_compile<- parallel::makeCluster(no_cores)
	  	  # predicted <- parallel::parSapply(cl_compile,1:nrow(iterfOutput$membership), fun_pred,Objyvar,ObjMembership,iterfOutput$membership,Objinbag, !(OOB))
	  ...
	  predicted <- fun_pred_SurvCat(iextractBBOP(Objyvar,ObjMembership,iterfOutput$membership,Objinbag,!(OOB),obsUniqueInTree))
	}
  if (iterfOutput$family == "HET" || iterfOutput$family == "indLSC" || iterfOutput$family == "indTSTC" || iterfOutput$family == "indTST2C" || iterfOutput$family == "indMaxLC" || iterfOutput$family == "indNSRC" || iterfOutput$family == "indMOB0C" || iterfOutput$family == "CMB")
  {
    Betas <- iextractBeta(Objyvar,ObjMembership,iterfOutput$membership,Objinbag,!(OOB),obsUniqueInTree)
    predicted <- Betas$b1lin
    
  }
  if (iterfOutput$family == "indQuadC" || iterfOutput$family == "indQuad2C" || iterfOutput$family == "indMOBQC"  || iterfOutput$family == "indMaxQC" || iterfOutput$family == "indMaxMaxQC")
  {
    Betas <- iextractBeta(Objyvar,ObjMembership,iterfOutput$membership,Objinbag,!(OOB),obsUniqueInTree)
    predicted <- get.max.effect(Betas)
  }

  if (iterfOutput$family == "indCat")
  {
    predicted <- fun_pred_Categorical(iextractBBOP(Objyvar,ObjMembership,iterfOutput$membership,Objinbag,!(OOB),obsUniqueInTree))
  }
  ...
  remove(predicted)
	...
	##Sami
	if (grepl("IndSurvB", iterfOutput$family) || grepl("HET", iterfOutput$family) || grepl("indQuadC", iterfOutput$family) || grepl("indQuad2C", iterfOutput$family) || grepl("indLSC", iterfOutput$family) || grepl("indTSTC", iterfOutput$family) || grepl("indTST2C", iterfOutput$family) || grepl("indNSRC", iterfOutput$family)  || grepl("indMOB0C", iterfOutput$family) || grepl("CMB", iterfOutput$family) || grepl("indMOBQC", iterfOutput$family) || grepl("indMaxLC", iterfOutput$family) || grepl("indMaxQC", iterfOutput$family) || grepl("indMaxMaxQC", iterfOutput$family) || grepl("indCat", iterfOutput$family) || grepl("IndSurvCat", iterfOutput$family))
	{
		predicted.oob <- NULL
	}
