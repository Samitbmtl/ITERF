           ...
           ...)
{
  ...
  if (!missing(object)) {
    ...
    ## determine the grow interface - iterf or iterfFast?
    if (!fast) {
      iterf.grow <- "iterf"
    }
    else {
      iterf.grow <- "iterfFast"
    }
  }
  ## --------------------------------------------------------------
  ##   
  ...
  if (missing(object)) {
    ...
    if (oob) {
      ...
      ## the resulting inbag sample depends on whether fast iterf is requested
      roughO <- do.call(iterf.grow,
