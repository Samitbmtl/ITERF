tune.iterf <- function(formula, data,
      ...
      trace = FALSE, doBest = TRUE, ...) 
{
  ...
  for (nsz in nodesizeTry) {
    ...
    o <- iterf.fast(formula, data, ntree = ntreeTry, mtry = mtryStart, nodesize = nsz, sampsize = sampsize, ...)
    mtryStart <- o$mtry
    ...
    for (direction in c("left", "right")) {
      ...
      while (counter2 <= maxIter && (Improve >= improve || (Improve < 0 && strikes < strikeout))) {
        ...
        errorCur <- mean(get.mv.error(iterf.fast(formula, data, ntree = ntreeTry, 
  ...
  rf <- NULL
  if (doBest) {
    rf <- iterf.fast(formula, data, mtry = res[opt.idx, 2], nodesize = res[opt.idx, 1], sampsize = sampsize)
  }
  ## return the goodies
